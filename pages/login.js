...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.success) {
        ...
        const userRes = await fetch(`${process.env.NEXT_PUBLIC_API}/api/Get/getusers`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
  ...
  return (
    <>
      <ToastContainer
        ...
        theme="light"
      />
      {loading && (name !== "" || password !== "" || email !== "") && <Loader />}
        ...
        <div className='absolute inset-0 flex justify-center items-center'>
          <form onSubmit={handleSubmit}>
            <div className='flex flex-col lg:left-[35vw]  lg:top-[25vh] rounded-lg p-12 '>
              <h1 className='font-bold text-3xl text-white py-4 text-center '>Login</h1>
  ...
  <Link href={'/resetPassword'}><div className='lg:text-black text-white font-semibold px-1 mt-2 underline' >Forget Password? </div></Link>
