...
const OpenWorld = ({OpenWorld}) => {
  return (
    <>
          <div>
            {OpenWorld.length==0 && <div>No Games To Show</div>}
            {OpenWorld && OpenWorld.map((game,index)=>(
...
export async function getServerSideProps(context) {
  let OpenWorld = await game.find({ category: "OpenWorld" })
  return {
    props: { OpenWorld: JSON.parse(JSON.stringify(OpenWorld)) }, // will be passed to the page component as props
  }
}
